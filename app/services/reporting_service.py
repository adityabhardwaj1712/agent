...
class ReportingService:
    ...
    async def generate_daily_report(self) -> str:
        ...
        async with async_session_factory() as db:
            ...
            # 2. Top Agents
            stmt = select(Agent.name, func.count(Task.task_id).label("count")).select_from(Agent).join(Task, Agent.agent_id == Task.agent_id).filter(Task.created_at >= yesterday).group_by(Agent.name).order_by(func.count(Task.task_id).desc()).limit(3)
