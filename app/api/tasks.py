...
def run(
    ...
    log_audit(db, request=request, agent_id=current.agent_id, action="tasks.run", status_code=200)
    return result
...
def status(
    ...
    log_audit(db, request=request, agent_id=current.agent_id, action="tasks.status", status_code=200)
    return result
